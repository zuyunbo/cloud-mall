...
public class GDispatcherServlet extends HttpServlet {
    ...
    private void doDispatch(HttpServletRequest req,HttpServletResponse response) throws IOException, InvocationTargetException, IllegalAccessException {
        ...
        method.invoke(o ,new Object[]{req,response,params.get("name")[0]});
            ...
            for (String className : mapping.keySet()) {
                ...
                if(clazz.isAnnotationPresent(GController.class)){
                    ...
                    if (clazz.isAnnotationPresent(GRequestMapping.class)) {
                        GRequestMapping requestMapping = clazz.getAnnotation(GRequestMapping.class);
                        baseUrl = requestMapping.value();
                    }
                    Method[] methods = clazz.getMethods();
                    for (Method method : methods) {
                        if (!method.isAnnotationPresent(GRequestMapping.class)) {  continue; }
            ...
            for (Object object : mapping.values()) {
                ...
                if(clazz.isAnnotationPresent(GController.class)){
                    ...
                    for (Field field : fields) {
                        if(!field.isAnnotationPresent(GAutowired.class)){continue; }
                        GAutowired autowired = field.getAnnotation(GAutowired.class);
